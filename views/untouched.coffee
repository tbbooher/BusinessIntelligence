(->
  ...
  drawMap = ->
    ...
    displayCurrentSettings()
  drawTopFiveInfoText = (text) ->
    topFiveInfo.select("body").html "<p>" + text + "</p>"
  drawSpark = ->
    if selectedCounty or currentCounty
      ...
      while i < years.length
        ...
        i++
      spark.selectAll("path").data([ sparkdata ]).attr "d", sparkline
      spark.selectAll("circle").data([ sparkdata[currentYearIndex] ]).attr("cx", (d, i) ->
        sparkx currentYearIndex
      ).attr "cy", (d, i) ->
        sparky (if d then d else minValue)

      spark.selectAll("text").data([ sparkdata[0], sparkdata[sparkdata.length - 1], sparkdata[currentYearIndex] ]).attr("x", (d, i) ->
        (if i < 2 then i * (sparkLineWidth - 22) - 8 else sparkx(currentYearIndex))
      ).attr("y", (d, i) ->
        (if i < 2 then sparky((if d then d else minValue)) + 5 else sparky((if d then d else minValue)) - 10)
      ).text (d, i) ->
        (if i < 2 then (if d then d3.format(".1f")(d) else "n/a") else (if currentYearIndex > 0 and currentYearIndex < years.length - 1 then (if d then d3.format(".1f")(d) else "n/a") else ""))

      countyName.selectAll("text").data(name).text (d, i) ->
        d
    else
  ...
  setTopFive = ->
    ...
    while i < years.length
      ...
      i++
    topFiveData = d3.nest().key((d) ->
      d.key
    ).entries(d3.merge(allData)).map((value) ->
      allValues = value.values.map((value) ->
        value.value
      )
  ...
  yearLeft = ->
    currentYearIndex--
    currentYearIndex = years.length - 1  if currentYearIndex < 0
    drawMap()
    drawSpark()
    d3.select(selectedCounty).style "fill", highlightColor  if selectedCounty
  yearRight = ->
    currentYearIndex++
    currentYearIndex = 0  if currentYearIndex > years.length - 1
    drawMap()
    drawSpark()
    d3.select(selectedCounty).style "fill", highlightColor  if selectedCounty
  data = undefined
